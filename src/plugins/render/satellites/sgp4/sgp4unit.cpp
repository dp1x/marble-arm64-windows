     ...
     )
{
    UNUSED( inclo );
     /* --------------------- local variables ------------------------ */
     const double twopi = 2.0 * marble_pi;
     ...
     )
{
     ...
     if ((inclm < 5.2359877e-2) || (inclm > marble_pi - 5.2359877e-2))
       shs = 0.0;
     if (sinim != 0.0)
       shs = shs / sinim;
     sgs  = sghs - cosim * shs;
     ...
     if ((omeosq >= 0.0 ) || ( satrec.no >= 0.0))
       {
         ...
         /* --------------- deep space initialization ------------- */
         if ((2*marble_pi / satrec.no) >= 225.0)
           {
             satrec.method = 'd';
             satrec.isimp  = 1;
             tc    =  0.0;
     ...
     if (satrec.method == 'd')
       {
         ...
         if (xincp < 0.0)
           {
             xincp  = -xincp;
             nodep = nodep + marble_pi;
             argpp  = argpp - marble_pi;
           }
         if ((ep < 0.0 ) || ( ep > 1.0))
           {
//            printf("# error ep %f\n", ep);
...
double  gstime
   ...
   {
     const double twopi = 2.0 * marble_pi;
     const double deg2rad = marble_pi / 180.0;
     double       temp, tut1;
